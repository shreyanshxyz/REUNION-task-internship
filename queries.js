...
const addPost = async (userid, post) => {
  return pool
    .query(
      ...
      [post.title, post.description, userid]
    )
    .then((res) => {
...
const deletePost = async (postid, userid) => {
  return pool
    .query(
      `
    DELETE FROM posts where userid = $1 AND id = $2`,
      [userid, postid]
    )
    .then((result) => {
...
const likePost = async (postid, userid) => {
  return pool
    .query("INSERT INTO likes VALUES($1, $2)", [postid, userid])
    .then(() => {
...
const followUser = async (followingid, followerid) => {
  ...
  return pool
    .query("INSERT INTO FOLLOWERS VALUES($1, $2)", [followingid, followerid])
    .then(() => {
...
const unfollowUser = async (followingid, followerid) => {
  ...
  return pool
    .query("DELETE FROM FOLLOWERS WHERE userid=$1 and followerid=$2", [
      followingid,
      followerid,
    ])
    .then((results) => {
...
const comment = async (postid, userComment, userid) => {
  ...
  return pool
    .query(
      "INSERT INTO comments(postid, userid, comment) VALUES($1, $2, $3) RETURNING id;",
      [postid, userid, userComment]
    )
    .then((results) => {
      return {
        message: `Successfully added the comment on the post ${postid}`,
        Comment_id: results.rows[0],
      };
    })
    .catch((e) => {
      if (e.code === "23503") {
...
const authenticateUser = (user) => {
  ...
  return pool
    .query(`SELECT name, id, password FROM users WHERE email = $1;`, [
      user.email,
    ])
    .then((results) => {
      if (results.rowCount === 0) {
